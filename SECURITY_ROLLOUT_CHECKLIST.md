...
| R2 | Phase 1B — Session foundation | เพิ่ม server session และ auth helper กลาง | 2 | Backend และ R2E controlled verification/manual gate ผ่าน; รออนุมัติ production deploy | R1 |
...
|---|---|
| Active release | R2 |
| Active phase | R2E verification เสร็จสมบูรณ์ — รออนุมัติ production deploy |
| เป้าหมาย | ปิด R2 session foundation gate และเตรียม safe production checkpoint โดยยังคง R1 containment บน protected RPC |
| สถานะ implementation | แก้ `index.html` และเพิ่ม `tests/r2e_frontend_session.test.js`; ไม่แก้ `Code.js` |
| สถานะ review | R2E ผ่าน 9/9, R2D regression ผ่าน 14/14 และไม่พบ Critical/High finding ใหม่ |
| สถานะ commit | `0d8d96e56613f1e0f54b712bb7e53ae19c25f9f3` — `security: retain frontend sessions in memory` |
| สถานะ Apps Script source | R2E ถูก controlled `clasp push` ไป Apps Script HEAD แล้วจาก sandbox; pull กลับมาตรง 7/7 ไฟล์; production ยังเป็น R1 |
| สถานะ verification | R2E local gate 9/9, R2D regression 14/14, remote verification 7/7 และ `/dev` manual gate ผ่าน |
| สถานะ Manual Test Gate | ผ่านเมื่อ 2026-06-20: login/logout/session memory และ R1 containment ทำงานตามคาด |
| สถานะ deployment | R1 อยู่บน production; R2 ยังไม่ได้ deploy |
| Blocker | ไม่มี blocker สำหรับ R2 production checkpoint; Task ownership และ Export permission ยัง block R3 และ password hashing migration ยังต้องวางแผนแยก |
| งานถัดไป | ขออนุมัติ production deploy ของ R2 foundation ผ่าน Apps Script UI แล้วทำ production smoke test; ยังไม่เปิด protected RPC |
...
สถานะ: R2A–R2E verification และ manual gate ผ่านแล้ว; รออนุมัติ production deploy โดยยังคง R1 containment
...
สถานะ: เริ่มบางส่วนผ่าน R2E โดยยังไม่ส่ง token ไป protected business RPC
...
| อนุมัติ deploy R2 | เจ้าของโครงการ | รออนุมัติ | — | Deploy เป็น production checkpoint โดยยังไม่เปิด protected RPC |
