...
| R2 | Phase 1B — Session foundation | เพิ่ม server session และ auth helper กลาง | 2 | R2A–R2C hardening commit แล้ว; รอ R2D test gate | R1 |
...
|---|---|
| Active release | R2 |
| Active phase | R2C auth hardening เสร็จ; งานถัดไป R2D — Foundation security test gate |
| เป้าหมาย | จำกัด throttle storage/timing leakage และลด admin permission ตาม least privilege |
| สถานะ implementation | R2A–R2C hardening เสร็จแล้วใน `Code.js`; ยังไม่ผูก permission helper เข้ากับ protected endpoints |
| สถานะ review | Storage abuse, digest path, permission regression และ session regression tests ผ่าน |
| สถานะ commit | `906675c38fdf6a8083ce119b7e1c0a8940d33273` — `security: harden login throttling` |
| สถานะ Apps Script source | Production ยังเป็น R1; R2 ยังไม่ได้ `clasp push` |
| สถานะ verification | random-username storage cap, timing path, login/session และ least-privilege tests ผ่าน |
...
สถานะ: กำลังดำเนินการ — R2A–R2C hardening เสร็จและ commit แล้ว; R2D–R2E ยังไม่เริ่ม
