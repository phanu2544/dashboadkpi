...
| R2 | Phase 1B — Session foundation | เพิ่ม server session และ auth helper กลาง | 2 | Backend runtime gate ผ่าน; R2E frontend session integration commit แล้วและรอ controlled Apps Script verification | R1 |
...
|---|---|
| Active release | R2 |
| Active phase | R2E — Minimal frontend session integration |
| เป้าหมาย | เก็บ server-issued token ใน memory, ใช้ server-derived user, revoke ตอน logout และจัดการ expiry โดยยังไม่ส่ง token ไป protected RPC |
...
| สถานะ Apps Script source | R2 backend ถูก controlled `clasp push` ไป Apps Script HEAD แล้ว; R2E frontend commit นี้ยังไม่ได้ `clasp push`; production ยังเป็น R1 |
| สถานะ verification | R2 backend runtime/manual gate ผ่าน; R2E local static/security gate ผ่าน 9/9 |
| สถานะ Manual Test Gate | R2 backend `/dev` ผ่าน; R2E browser manual gate รอหลัง controlled sandbox push |
| สถานะ deployment | R1 อยู่บน production; R2 ยังไม่ได้ deploy |
| Blocker | ไม่มี blocker สำหรับ R2E local implementation; Task ownership และ Export permission ยัง block R3 และ password hashing migration ยังต้องวางแผนแยก |
| งานถัดไป | Push GitHub branch สำหรับ review แล้วเตรียม controlled Apps Script sandbox verification ของ R2E; ยังไม่ deploy production |
...
สถานะ: R2A–R2D และ Apps Script backend runtime/manual gate ผ่านแล้ว; R2E frontend session integration commit แล้วแต่ยังไม่ `clasp push` หรือ deploy
...
- [ ] ทำ controlled Apps Script verification สำหรับ R2E frontend integration
...
- `clasp push`/deployment สำหรับ R2E frontend commit: ยังไม่ได้ดำเนินการ
- Rollback target: R1 containment
- งานถัดไป: GitHub review แล้วทำ controlled Apps Script verification จาก sandbox ก่อนพิจารณา deploy
