...
| R3 | Phase 1C — Server protection | ป้องกัน endpoint สำคัญด้วย permission และ resource scope | 3 | R3A policy/schema decision อนุมัติแล้ว; ยังไม่เริ่มแก้ application code | R2 production checkpoint |
...
|---|---|
| Active release | R3 |
| Active phase | R3A — Task ownership / Export policy approved |
| เป้าหมาย | เตรียม R3B authorization helpers และ migration design โดยยังไม่เปิด protected endpoint |
| สถานะ implementation | บันทึก policy/schema/default-deny decisions แล้ว; ยังไม่แก้ application code |
| สถานะ review | R3A policy ผ่าน review; พบว่าต้องเพิ่ม immutable task assignee ID และ user scope mapping ก่อนเปิด read/write |
| สถานะ commit | R2 application commit `0d8d96e56613f1e0f54b712bb7e53ae19c25f9f3`; production source อยู่บน merge commit `21ba1c34fad78b8c39dc5ee1d5807ad3e9cea608` |
...
| Blocker | Ownership/export policy ไม่เป็น blocker แล้ว; ต้องตรวจข้อมูลจริงเพื่อวาง migration `AssigneeUserId`/`UserScope` แบบไม่เดาสุ่ม และ password hashing migration ยังแยกต่างหาก |
| งานถัดไป | R3B เพิ่ม authorization/scope helpers และ automated tests โดยคง R1 maintenance guard ไว้ทั้งหมด |
...
- [ ] กำหนด role grants สำหรับ Task/Export ตาม R3A policy
...
| R3 permission model | Codex | R3A เสร็จ; R3B รอเริ่ม | 2026-06-21 | ยังไม่เปิด endpoint และยังไม่แก้ application code |
