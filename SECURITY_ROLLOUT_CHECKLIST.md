...
| R2 | Phase 1B — Session foundation | เพิ่ม server session และ auth helper กลาง | 2 | กำลังดำเนินการ — R2A/R2B commit แล้ว; ยังไม่ `clasp push`/deploy | R1 |
...
|---|---|
| Active release | R2 |
| Active phase | R2B เสร็จ; งานถัดไป R2C — Permission foundation |
| เป้าหมาย | ออก session จาก login, รองรับ server logout/context และลด brute-force โดยยังไม่เปิด protected endpoint กลับมา |
| สถานะ implementation | R2A/R2B เสร็จแล้วใน `Code.js`; frontend ยังไม่เก็บหรือส่ง token |
| สถานะ review | Static checks และ in-memory login/session/throttling tests ผ่าน |
| สถานะ commit | `172de5d46fa3210a6ebe7cf8243ee5e33e2fc779` — `security: issue sessions from login` |
| สถานะ Apps Script source | Production ยังเป็น R1; R2 ยังไม่ได้ `clasp push` |
| สถานะ verification | login/context/logout, role refresh, generic failure, lockout และ duplicate username ผ่าน |
| สถานะ Manual Test Gate | ยังไม่เริ่มสำหรับ R2 |
| สถานะ deployment | R1 อยู่บน production; R2 ยังไม่ได้ deploy |
| Blocker | R2C ต้องกำหนด permission map; password hashing migration ยังต้องวางแผนแยก |
| งานถัดไป | Review PR/merge R2B แล้วออกแบบ `requirePermission_`; ยังไม่ `clasp push`/deploy |
...
สถานะ: กำลังดำเนินการ — R2A/R2B เสร็จและ commit แล้ว; R2C–R2E ยังไม่เริ่ม
