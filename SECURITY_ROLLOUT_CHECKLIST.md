...
| R2 | Phase 1B — Session foundation | เพิ่ม server session และ auth helper กลาง | 2 | เสร็จสมบูรณ์ — deploy และผ่าน Production Smoke Test แล้ว | R1 |
| R3 | Phase 1C — Server protection | ป้องกัน endpoint สำคัญด้วย permission และ resource scope | 3 | R3A policy และ R3B authorization foundation เสร็จแล้ว; read endpoints ยังไม่ protected | R2 production checkpoint |
| R4 | Phase 1D — Frontend token migration | เปลี่ยน frontend ให้ใช้ token และเลิกส่ง identity claim | 4 | เริ่มบางส่วนผ่าน R2E: memory token, server user, logout และ expiry; protected RPC migration ยังไม่เริ่ม | R1 โดยปิด protected features ไว้ |
...
|---|---|
| Active release | R3 |
| Active phase | R3B — Authorization foundation เสร็จ; เตรียม R3C read protection |
| เป้าหมาย | ป้องกัน read endpoints และ migrate frontend read RPC ให้ส่ง session token โดยไม่เปิด mutation/export |
| สถานะ implementation | เพิ่ม private task scope/export authorization helpers และ role grants; ไม่มี schema/data mutation |
| สถานะ review | R3B 10/10, R2D 14/14, R2E 9/9 ผ่าน; R1 guards ยังคง 40 จุด |
| สถานะ commit | `458030429b01cfb9a058eb7546a5439f3eb3ef59` — `security: add task scope authorization foundation` |
| สถานะ Apps Script source | R2 foundation อยู่บน production; protected business RPC ยังคงถูก R1 containment บล็อก |
| สถานะ verification | R2E 9/9, R2D 14/14, remote 7/7, `/dev` manual gate และ Production Smoke Test ผ่าน |
| สถานะ Manual Test Gate | R2 `/dev` และ production ผ่าน |
| สถานะ deployment | R2 production deployment สำเร็จเมื่อ 2026-06-21 |
| Blocker | ก่อนเปิด row-level scoped read สำหรับ user/admin ต้องมี `UserScope`; ก่อนเปิด user mutation ต้องมี `AssigneeUserId`; password hashing migration ยังแยกต่างหาก |
| งานถัดไป | R3C ป้องกัน `getTasks`/read endpoints และ frontend token migration สำหรับ read; เริ่มด้วย superadmin/admin-safe path และคง mutation/export guard |
...
สถานะ: เสร็จสมบูรณ์ — production deployment และ smoke test ผ่านเมื่อ 2026-06-21 โดยยังคง R1 containment บน protected business RPC
...
- [x] เพิ่ม local automated security suite โดยไม่เพิ่ม Apps Script test RPC
- [x] R2D automated foundation security gate ผ่าน 14/14
- [x] ทำ Apps Script runtime/manual verification สำหรับ R2 backend foundation
- [x] เก็บ session token ใน frontend memory และใช้ server-derived user
- [x] Revoke server session เมื่อ logout
- [x] จัดการ absolute/idle expiry และ revalidate server session แบบ throttled
- [x] ทำ controlled Apps Script verification สำหรับ R2E frontend integration
...
สถานะ: R3A policy/schema decision อนุมัติแล้ว; ยังไม่แก้ application code และยังไม่ส่ง token ไป protected business RPC

- [ ] ป้องกัน critical endpoints ด้วย session validation
- [x] มี server-side permission map foundation แบบ default-deny จาก R2
- [x] กำหนด role grants สำหรับ Task/Export ตาม R3A policy
...
- [x] ตัดสินใจไม่เปิด public Drive-link export

#### R3A — Policy ที่อนุมัติแล้ว

Task read:

- [x] `user` อ่านรายการ Task ภายใน organizational scope ของตน
- [x] `admin` อ่านรายการ Task ภายใน scope ที่กำหนดให้
- [x] `superadmin` อ่านได้ทุก Task
- [x] Dashboard aggregate สำหรับ user ต้องไม่เปิดเผย row-level data นอก scope; หากแยก aggregate อย่างปลอดภัยไม่ได้ ให้กรองตาม scope เช่นเดียวกับรายการ Task

Task mutation:

- [x] `user` แก้ได้เฉพาะผลรายเดือน/การส่งข้อมูลของ Task ที่ `AssigneeUserId` ตรงกับ user ID ใน validated session
- [x] `user` ห้ามสร้าง ลบ เปลี่ยน assignee หรือแก้ metadata/ownership
- [x] `admin` สร้างและแก้ Task ได้เฉพาะภายใน scope และ assign ได้เฉพาะ user ใน scope
- [x] `admin` ยังไม่ให้ลบ Task ใน rollout แรก
- [x] `superadmin` จัดการ Task ได้ทุก scope รวมถึงการลบ
- [x] `SubmittedBy`, `CancelledBy`, creator และ audit actor ต้องมาจาก validated session เท่านั้น

Export:

- [x] Summary export: `admin` ภายใน scope และ `superadmin` ทุก scope
- [x] Raw export รุ่นแรก: `superadmin` เท่านั้น
- [x] `user` ไม่มี export permission ใน rollout แรก
- [x] Export คืน CSV content ให้ browser ดาวน์โหลดเป็น local file
- [x] ห้ามสร้าง Drive file และห้ามคืน public/download URL
- [x] ต้องป้องกัน CSV formula injection สำหรับค่าที่ขึ้นต้นด้วย `=`, `+`, `-`, `@`
- [x] Filter จาก browser เป็นเพียงคำขอ; server ต้องบังคับ actor scope ซ้ำเสมอ

#### R3A — Schema decision

- [x] ใช้ `AssigneeUserId` เป็น immutable authorization key ของ Task
- [x] คง `Assignee` เดิมไว้เป็น display label เท่านั้น ห้ามใช้ตัดสินสิทธิ์
- [x] ใช้ชีต `UserScope` แยกจากชีต `login` เพื่อ map immutable `UserId` กับ `Department`, `WorkGroup`, `SubDep`
- [x] รองรับหลาย scope ต่อผู้ใช้ด้วยหนึ่งแถวต่อหนึ่ง scope assignment
- [x] ระบุสถานะ active/inactive ของ scope assignment
- [x] หาก Task ไม่มี `AssigneeUserId` ให้ default-deny mutation จนกว่าจะ map สำเร็จ
- [x] หาก user/admin ไม่มี scope ที่ valid ให้ default-deny row-level read/write
- [x] `superadmin` ไม่ต้องพึ่ง scope row สำหรับ global access แต่ยังต้องผ่าน session/permission validation

โครงสร้าง `UserScope` ขั้นต่ำที่วางแผนไว้:

| Field | ใช้ทำอะไร |
|---|---|
| `UserId` | immutable ID จากชีต login |
| `Department` | ขอบเขตระดับหมวดหมู่ |
| `WorkGroup` | ขอบเขตระดับกลุ่มงาน |
| `SubDep` | ขอบเขตระดับหน่วยย่อย; ว่างได้เมื่อ scope อยู่ระดับบน |
| `Active` | เปิด/ปิด scope assignment |

#### R3A — Migration safety

- [ ] ทำ inventory แบบ read-only ของค่า `Assignee`, `Department`, `WorkGroup`, `SubDep` และ user ID ก่อนสร้าง mapping
- [ ] สร้าง migration preview ที่รายงาน matched / ambiguous / unmatched โดยไม่เขียนข้อมูล
- [ ] ห้าม map ด้วยชื่อแบบเดาเมื่อมีชื่อซ้ำหรือไม่พบ username/user ID ที่แน่นอน
- [ ] ให้เจ้าของโครงการยืนยัน ambiguous/unmatched rows ก่อน migration จริง
- [ ] ทำ backup ก่อนเพิ่ม `AssigneeUserId` หรือสร้าง `UserScope`
- [ ] Migration จริงต้อง idempotent และมี dry-run
- [ ] Task เก่าที่ยัง map ไม่ได้ต้องอ่าน/แก้ได้เฉพาะ `superadmin` จนกว่าจะ resolve

#### ลำดับ R3 ที่อนุมัติ

1. R3B — เพิ่ม private authorization/scope helpers และ tests; คง R1 guard
2. R3C — ป้องกัน read endpoints และ frontend token migration สำหรับ read
3. R3D — เปิด Summary CSV content export สำหรับ admin/superadmin ตาม scope
4. R3E — เปิด Task monthly input/submit/cancel ก่อน แล้วจึง create/metadata/delete ทีละชุด
5. R3F — พิจารณา Raw export สำหรับ superadmin หลัง security tests ผ่าน

Acceptance criteria ก่อนเปิด endpoint แรก:

- [ ] ไม่มี endpoint เชื่อ `currentUsername`, `role`, `submittedBy`, `cancelledBy` หรือ assignee จาก browser
- [ ] ทุก public wrapper รับ session token และดึง actor ด้วย `requirePermission_`
- [ ] Scope filter ทำบน server ก่อนคืนข้อมูลหรือ mutate
- [ ] Unknown/missing scope และ missing `AssigneeUserId` ต้อง fail closed
- [ ] Automated tests ครอบคลุม user/admin/superadmin, cross-scope access, forged identity และ ambiguous migration
- [ ] R1 maintenance guard ถูกเอาออกเฉพาะ endpoint ที่ review/test ผ่านแล้ว

#### R3B — Authorization Foundation

- [x] เพิ่ม `AUTH_TASK_ACTIONS` สำหรับ read/monthly input/submit/cancel/create/metadata/assign/delete
- [x] เพิ่ม role grants:
  - user: `tasks:read`, `tasks:mutate`
  - admin: `tasks:read`, `tasks:mutate`, `export:summary`
  - superadmin: `tasks:read`, `tasks:mutate`, `export:summary`, `export:raw`
- [x] เพิ่ม private `getUserScopeRecords_` แบบ read-only
- [x] `UserScope` schema ขาด/ซ้ำต้อง fail closed
- [x] inactive scope และ scope ที่ไม่มี `UserId`/`Department` ไม่ถูกนำมาใช้
- [x] เพิ่ม `getActorScopes_`, `taskMatchesActorScope_`, `filterTasksForActor_`
- [x] เพิ่ม `canReadTask_`, `canMutateTask_`, `requireTaskAccess_`
- [x] user mutation ต้องมี scope, `AssigneeUserId` ตรง actor และ action อยู่ใน allowlist
- [x] admin mutation ต้องอยู่ใน scope และห้าม delete ใน rollout แรก
- [x] superadmin มี global access แต่ unknown action/missing Task ID ยัง fail closed
- [x] เพิ่ม `requireExportPermission_`; admin summary ต้องมี valid scope
- [x] ไม่มี helper ใดสร้างชีต เพิ่ม header หรือเขียนข้อมูล
- [x] ไม่มี public R3 RPC ใหม่
- [x] R1 maintenance guards ยังคงอยู่ครบ
- [x] R3B tests ผ่าน 10/10
- [x] R2D regression ผ่าน 14/14
- [x] R2E regression ผ่าน 9/9

ข้อจำกัดที่ต้องแก้ใน R3C:

- `getTasks`, `getTasksByYear`, task history และ monthly closed read endpoints ยังไม่ได้รับ session token/scope enforcement
- `UserScope` และ `AssigneeUserId` ยังไม่ได้สร้างหรือ migrate ในข้อมูลจริง
- Role grant อย่างเดียวไม่เพียงพอ; public wrapper ต้องใช้ `requireTaskAccess_`/server-side filtering ทุกครั้ง
- Mutation และ export ยังคงต้องถูก R1 maintenance guard ปิดไว้
...
- [x] เก็บ session token ใน memory สำหรับ rollout แรก
...
- active session user มาจาก `response.user` ของ server แบบ fail-closed
- client จัดการ absolute/idle expiry และ revalidate ทุก 5 นาทีเมื่อมี activity
- token ถูกส่งเฉพาะ `getSessionContext` และ `logoutSession`
- protected business RPC ยังคงถูก R1 containment ปิดไว้ และยังไม่รับ token
...
- Permission model:
  ...
  - `admin`: ดู admin console/monthly-close status และทำ monthly-close dry run
...
### 2026-06-19 — R2C Authentication Hardening

- ขอบเขต: hardening ก่อน R2D โดยไม่เพิ่ม public RPC และไม่เปิด protected endpoint
- Commit: `906675c38fdf6a8083ce119b7e1c0a8940d33273`
- Commit message: `security: harden login throttling`
- ไฟล์ที่แก้: `Code.js`
- Throttle storage:
  - บัญชีจริงใช้ throttle subject จาก immutable user ID
  - username ที่ไม่มีอยู่หรือซ้ำใช้ shared unknown-user bucket
  - username สุ่มจำนวนมากไม่สร้าง property ใหม่ต่อ username
  - จำกัด throttle records สูงสุด 250 รายการ
  - บังคับเพดานแม้ request อยู่ในเส้นทาง lockout
- Timing-path hardening:
  - known และ unknown username ทำ password digest path จำนวนเท่ากัน
  - unknown username ใช้ dummy password constant สำหรับ digest comparison
- Least privilege:
  - ถอน `SYSTEM_CONFIG_VIEW` และ `SNAPSHOT_VIEW` จาก role `admin`
  - `admin` ยังคง admin console/monthly-close view และ dry-run permission
  - config/snapshot view และ management คงไว้เฉพาะ `superadmin`
- การทดสอบ:
  - username สุ่ม 100 ค่าใช้ shared unknown bucket เดียว
  - throttle records ถูกจำกัดไม่เกิน 250 แม้เข้าสู่ locked path
  - known/unknown digest call count เท่ากัน
  - successful login ล้าง throttle ของบัญชี
  - login → context → logout regression ผ่าน
  - admin ถูกปฏิเสธ config/snapshot แต่ยังทำ monthly-close dry run ได้
  - superadmin ยังดู system config ได้
  - JavaScript syntax และ diff checks ผ่าน
  - ไม่มี public RPC ใหม่และไม่พบ sensitive auth logging
  - R1 maintenance early return ยังคง 31 จุดเท่าเดิม
- Review: ไม่พบ Critical/High issue ใหม่
- ความเสี่ยงคงเหลือ:
  - targeted account lockout ยังเป็นไปได้ภายใต้ anonymous deployment
  - plaintext password migration ยังไม่ได้ดำเนินการ
- `clasp push`/deployment: ยังไม่ได้ดำเนินการสำหรับ R2
- Rollback target: R1 containment
- งานถัดไป: R2D regression/security test gate

### 2026-06-19 — R2D Automated Foundation Security Gate

- ขอบเขต: local automated tests สำหรับ R2 foundation และ R1 containment regression
- Commit: `36b36f4b47ca28390e1a868e6e256d56e15b9ec5`
- Commit message: `test: add R2D security regression suite`
- ไฟล์ที่เพิ่ม: `tests/r2d_security.test.js`
- Application code ที่แก้: ไม่มี
- วิธีทดสอบ:
  - ใช้ Node `vm` โหลด `Code.js` จริง
  - mock `PropertiesService`, `LockService`, `SpreadsheetApp` และ `Utilities`
  - ไม่อ่านหรือเขียน Script Properties/ชีตจริง
  - test file ไม่ถูกคัดลอกเข้า clasp sandbox
- ผลการทดสอบ: ผ่าน 14/14
- กรณีที่ครอบคลุม:
  - session token เก็บเฉพาะ hash
  - login/context/logout lifecycle
  - forged, expired และ revoked token
  - จำกัด 3 sessions และ revoke session เก่าสุด
  - role refresh จาก server
  - default-deny permission matrix
  - duplicate username และ immutable user ID fail closed
  - generic login failure และ successful-login throttle reset
  - known-account lockout หลังผิด 5 ครั้ง
  - shared unknown-user throttle bucket
  - throttle record cap บน locked path
  - known/unknown digest path count เท่ากัน
  - critical endpoints ยังคง R1 maintenance guard
  - ไม่พบ `ANYONE_WITH_LINK`
  - `doGet` ยังคง render-only
  - ไม่มี public test RPC
- Review:
  - `Code.js` ตรงกับ `main` และไม่เปลี่ยนใน branch นี้
  - ไม่เพิ่ม Apps Script RPC surface
  - ไม่พบ Critical/High finding ใหม่
- ข้อจำกัด:
  - เป็น local mocked-service gate ไม่ใช่ Apps Script runtime/manual test
  - password hashing migration ยัง pending
  - protected endpoints ยังไม่เปิดกลับ
- `clasp push`/deployment: ยังไม่ได้ดำเนินการ
- Rollback target: R1 containment
- งานถัดไป: merge R2D และจัดทำ controlled Apps Script runtime verification plan

### 2026-06-19 — R2 Backend Runtime Gate และ R2E Frontend Session Integration

- R2 backend controlled push:
  - Push จาก `C:\Users\User\testingClash_push_r2` เท่านั้น
  - ไม่ใช้ `--force`
  - Apps Script remote source ตรงกับ sandbox 7/7 ไฟล์
  - ไม่มี deploy production
- R2 backend runtime/manual gate:
  - login สำเร็จและ login ผิดคืนผลตามคาด
  - R1 protected features ยังคงถูกบล็อก
  - Task และ SystemConfig ไม่เปลี่ยน
  - Google Drive ไม่มีไฟล์ export/snapshot ใหม่
  - Production ยังคง R1
- R2E frontend implementation:
  - Commit: `0d8d96e56613f1e0f54b712bb7e53ae19c25f9f3`
  - Commit message: `security: retain frontend sessions in memory`
  - ไฟล์ที่แก้: `index.html`
  - ไฟล์ที่เพิ่ม: `tests/r2e_frontend_session.test.js`
  - เก็บ token ใน memory เท่านั้น
  - fail-closed หาก login response ไม่มี server session หรือ server user ที่ครบ
  - revalidate session แบบ throttled ระหว่าง activity และเมื่อ browser focus
  - revoke session ที่ server ตอน logout และล้าง local state เสมอ
  - ยังไม่ส่ง token ไป protected business RPC และไม่เปิด R1 guard
- ผลการทดสอบ:
  - R2E frontend static/security tests ผ่าน 9/9
  - R2D backend regression tests ผ่าน 14/14
  - HTML inline scripts ผ่าน syntax check
  - `Code.js` ไม่เปลี่ยน
- Review:
  - ไม่พบ Critical/High finding ใหม่
  - ไม่พบ token ใน local/session storage หรือ console log
  - token RPC จำกัดอยู่ที่ `getSessionContext` และ `logoutSession`
- Controlled `clasp push` สำหรับ R2E:
  - สำเร็จเมื่อ 2026-06-20 เวลา 16:29:10 จาก `C:\Users\User\testingClash_push_r2e`
  - ไม่ใช้ `--force`
  - จัดรูปแบบ manifest ใน sandbox ให้ตรง remote โดยไม่เปลี่ยน semantic
  - pull กลับมาตรวจแล้ว Apps Script HEAD ตรง sandbox 7/7 ไฟล์
- R2E `/dev` manual gate: ผ่านเมื่อ 2026-06-20
- Production deployment: ยังไม่ได้ดำเนินการ
- Rollback target: R1 containment
- งานถัดไป: ขออนุมัติ production deploy ของ R2 foundation และทำ production smoke test

### 2026-06-20 — R2E Controlled Push และ Manual Gate ผ่าน

- Source commit บน `main`: `21ba1c34fad78b8c39dc5ee1d5807ad3e9cea608`
- Push sandbox: `C:\Users\User\testingClash_push_r2e`
- Pre-push verification:
  - R2E tests ผ่าน 9/9
  - R2D regression ผ่าน 14/14
  - source ใน sandbox ตรง `main`
  - ไม่พบ `SELF_REGISTRATION_ENABLED = true`
  - ไม่พบ `ANYONE_WITH_LINK`
- Controlled `clasp push`:
  - สำเร็จ 7 ไฟล์เมื่อ 16:29:10
  - ไม่ใช้ `--force`
  - ไม่มี deploy
- Post-push verification:
  - pull Apps Script HEAD ลง verification folder แยก
  - remote source ตรง sandbox 7/7 ไฟล์
  - พบ R2E session markers ครบ
- ผล manual test จากเจ้าของโครงการ:
  - page refresh ต้อง login ใหม่ตาม memory-only design
  - login แสดง server-derived user/role ถูกต้อง
  - logout ล้าง session และไม่คืนสถานะเดิม
  - User Management, Export และ Monthly Close ยังคงถูก R1 containment บล็อก
  - Task และ SystemConfig ไม่เปลี่ยน
  - Google Drive ไม่มีไฟล์ export/snapshot ใหม่
- ผล: R2E Manual Test Gate ผ่าน
- Git commit/push เพิ่มเติม: ไม่ได้ดำเนินการในรอบบันทึกนี้
- Production deployment: ยังไม่ได้ดำเนินการ
- งานถัดไป: ขออนุมัติ deploy R2 foundation ผ่าน Apps Script UI และทำ production smoke test

### 2026-06-21 — R2 Production Deploy และ Smoke Test ผ่าน

- Release: R2 / Phase 1B — Session Foundation
- Deployment:
  - สร้าง Apps Script deployment version ใหม่ผ่าน Apps Script UI
  - คง deployment configuration เป็น Execute as Me / Anyone
  - ไม่มี `clasp push` เพิ่มในขั้น deployment
- Production Smoke Test:
  - login สำเร็จและแสดง server-derived user/role ถูกต้อง
  - refresh ต้อง login ใหม่ตาม memory-only token design
  - logout ล้าง session และไม่คืน authenticated state เดิม
  - login ผิดคืนข้อความทั่วไป
  - User Management, Export และ Monthly Close ยังคงถูก R1 containment บล็อก
  - Task และ SystemConfig ไม่เปลี่ยน
  - Google Drive ไม่มีไฟล์ export/snapshot ใหม่
  - ไม่พบ browser console error หรือ Apps Script execution failure ใหม่ที่เป็น blocker
- ผล: Production Smoke Test ผ่าน
- สถานะ R2: ปิดสมบูรณ์
- Safe rollback target: R1 containment
- Protected business RPC: ยังไม่เปิดกลับ
- Git commit/push เพิ่มเติม: ไม่ได้ดำเนินการในรอบบันทึกนี้
- งานถัดไป: R3 permission/resource-scope design หลังตัดสินใจ task ownership และ export permission

### 2026-06-21 — R3A Task Ownership และ Export Policy

- ขอบเขต: policy/schema decision เท่านั้น
- Application code ที่แก้: ไม่มี
- Task ownership:
  - ใช้ `AssigneeUserId` เป็น authorization key
  - user อ่านใน organizational scope และแก้ monthly result เฉพาะ Task ที่ assign ให้ตน
  - admin อ่าน/สร้าง/แก้ภายใน scope แต่ยังไม่ลบใน rollout แรก
  - superadmin เข้าถึงทุก scope
- Organizational scope:
  - ใช้ชีต `UserScope` keyed by immutable `UserId`
  - รองรับ Department / WorkGroup / SubDep และหลาย scope ต่อ user
  - missing/invalid scope ใช้ default-deny
- Export:
  - Summary: admin ตาม scope และ superadmin
  - Raw: superadmin เท่านั้นใน rollout แรก
  - คืน CSV content ให้ browser; ไม่สร้าง Drive file หรือ URL
- Audit identity:
  - `SubmittedBy`, `CancelledBy`, creator และ actor มาจาก validated session เท่านั้น
- Migration:
  - ต้องมี read-only inventory และ dry-run preview
  - ambiguous/unmatched rows ห้ามเดา mapping
  - Task ที่ยังไม่มี `AssigneeUserId` ให้ mutate ได้เฉพาะ superadmin
- Review: policy สอดคล้องกับ Execute as Me + Anyone และ R2 default-deny foundation
- Commit/push/`clasp push`/deployment: ไม่ได้ดำเนินการ
- งานถัดไป: R3B private authorization/scope helpers และ automated tests โดยยังคง R1 guards

### 2026-06-21 — R3B Authorization Foundation

- Commit: `458030429b01cfb9a058eb7546a5439f3eb3ef59`
- Commit message: `security: add task scope authorization foundation`
- ไฟล์ที่แก้:
  - `Code.js`
  - `tests/r2d_security.test.js`
- ไฟล์ที่เพิ่ม:
  - `tests/r3b_authorization.test.js`
- พฤติกรรมที่เพิ่ม:
  - role grants สำหรับ Task และ Export ตาม R3A
  - read-only `UserScope` loader แบบ schema fail-closed
  - task scope matching และ filtering
  - user/admin/superadmin mutation action policy
  - central task/export authorization helpers
- สิ่งที่ไม่ได้ทำ:
  - ไม่สร้าง `UserScope`
  - ไม่เพิ่ม `AssigneeUserId`
  - ไม่แก้ข้อมูลจริง
  - ไม่เปิด read/write/export endpoint ใหม่
  - ไม่ถอด R1 maintenance guard
- ผลการทดสอบ:
  - R3B authorization: 10/10
  - R2D security regression: 14/14
  - R2E frontend session regression: 9/9
  - รวม 33/33
- Review:
  - ไม่พบ Critical/High finding ใหม่
  - duplicate/missing scope schema, cross-scope access, missing assignee, forged token และ unknown action fail closed
  - `getTasks` และ read endpoints เดิมยังเป็น read gap ที่ต้องแก้ใน R3C
- `clasp push`/deployment: ไม่ได้ดำเนินการ
- Rollback target: R2 production checkpoint
- งานถัดไป: R3C read endpoint protection + frontend token migration โดยยังไม่เปิด mutation/export
...
| R2C auth hardening | Codex | ผ่าน review และ commit | 2026-06-19 | Commit `906675c38fdf6a8083ce119b7e1c0a8940d33273`; bounded throttle storage, equal digest path และลด admin permission |
| R2C hardening tests | Codex | ผ่าน | 2026-06-19 | random username abuse, locked-path cap, login/session regression และ least-privilege matrix ผ่าน |
| R2D automated security suite | Codex | ผ่านและ commit | 2026-06-19 | Commit `36b36f4b47ca28390e1a868e6e256d56e15b9ec5`; local Node test harness ไม่แตะ Apps Script data จริง |
| R2D automated test gate | Codex | ผ่าน 14/14 | 2026-06-19 | Session, throttle, permission และ R1 containment regression ผ่าน |
| R2 controlled `clasp push` | Codex | สำเร็จ | 2026-06-19 | Push จาก `C:\Users\User\testingClash_push_r2`; ไม่ใช้ `--force`; remote source ตรง 7/7 ไฟล์ |
| R2 Apps Script runtime gate | เจ้าของโครงการ | ผ่าน | 2026-06-19 | `/dev` login และ R1 containment ทำงานตามคาด; Task/SystemConfig/Drive ไม่เปลี่ยน |
| R2E frontend session integration | Codex | ผ่าน review และ commit | 2026-06-19 | Commit `0d8d96e56613f1e0f54b712bb7e53ae19c25f9f3`; memory-only token, server user, logout revoke และ expiry/revalidation |
| R2E frontend security tests | Codex | ผ่าน 9/9 | 2026-06-19 | Syntax, fail-closed login, storage/log leakage, stale response, expiry, revalidation และ logout ผ่าน |
| R2E controlled `clasp push` | Codex | สำเร็จ | 2026-06-20 | Push 7 ไฟล์จาก `C:\Users\User\testingClash_push_r2e`; ไม่ใช้ `--force`; pull กลับมาตรง 7/7 ไฟล์ |
| R2E manual test gate | เจ้าของโครงการ | ผ่าน | 2026-06-20 | Login/logout/session memory ผ่าน; R1 containment คงอยู่; Task/SystemConfig/Drive ไม่เปลี่ยน |
| อนุมัติ deploy R2 | เจ้าของโครงการ | อนุมัติแล้ว | 2026-06-21 | Deploy เป็น production checkpoint โดยยังไม่เปิด protected RPC |
| R2 deployment | เจ้าของโครงการ | สำเร็จ | 2026-06-21 | Deploy เวอร์ชันใหม่ผ่าน Apps Script UI; session foundation อยู่บน production |
| R2 Production Smoke Test | เจ้าของโครงการ | ผ่าน | 2026-06-21 | Login/logout/refresh/generic failure ผ่าน; R1 containment คงอยู่; Task/SystemConfig/Drive ไม่เปลี่ยน |
| ปิด R2 / Phase 1B | เจ้าของโครงการ | เสร็จสมบูรณ์ | 2026-06-21 | R2 เป็น production checkpoint; protected business RPC ยังไม่เปิด |
| R3A task ownership policy | เจ้าของโครงการ | อนุมัติแล้ว | 2026-06-21 | user อ่านใน scope/แก้เฉพาะ assigned Task; admin ภายใน scope; superadmin ทุก scope |
| R3A export policy | เจ้าของโครงการ | อนุมัติแล้ว | 2026-06-21 | Summary admin/superadmin; Raw superadmin; local CSV content เท่านั้น; ไม่สร้าง Drive file |
| R3A schema decision | Codex/เจ้าของโครงการ | พร้อมสำหรับ R3B design | 2026-06-21 | `AssigneeUserId` + `UserScope`; missing/ambiguous mapping default-deny |
| R3B authorization foundation | Codex | ผ่าน review และ commit | 2026-06-21 | Commit `458030429b01cfb9a058eb7546a5439f3eb3ef59`; private scope/task/export helpers |
| R3B automated tests | Codex | ผ่าน 10/10 | 2026-06-21 | role/scope/cross-scope/mutation/export/missing schema/forged session ผ่าน |
| R3 permission model | Codex | R3B เสร็จ; R3C รอเริ่ม | 2026-06-21 | Mutation/export ยังถูก contain; read endpoints ยังต้อง protect |
| R4 frontend migration | Codex/เจ้าของโครงการ | เริ่มบางส่วน | 2026-06-19 | R2E foundation เสร็จ; protected RPC token migration และ browser identity removal ยังไม่เริ่ม |
