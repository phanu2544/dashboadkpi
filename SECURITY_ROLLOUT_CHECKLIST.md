...
| R2 | Phase 1B — Session foundation | เพิ่ม server session และ auth helper กลาง | 2 | R2D automated security gate ผ่าน; รอ Apps Script runtime gate | R1 |
...
|---|---|
| Active release | R2 |
| Active phase | R2D automated foundation security gate ผ่าน |
| เป้าหมาย | ตรวจ regression/security ของ session, login, throttle, permission และ R1 containment โดยไม่แตะ Apps Script data จริง |
| สถานะ implementation | เพิ่ม local Node test harness ใน `tests/r2d_security.test.js`; ไม่แก้ `Code.js` |
| สถานะ review | Automated security tests ผ่าน 14/14 และไม่มี public test RPC |
| สถานะ commit | `36b36f4b47ca28390e1a868e6e256d56e15b9ec5` — `test: add R2D security regression suite` |
| สถานะ Apps Script source | Production ยังเป็น R1; R2 ยังไม่ได้ `clasp push` |
| สถานะ verification | R2D local automated gate ผ่าน 14/14 |
| สถานะ Manual Test Gate | Apps Script runtime/manual gate ยังไม่เริ่มสำหรับ R2 |
...
| งานถัดไป | Merge R2D PR แล้ววางแผน controlled Apps Script runtime verification; ยังไม่ deploy production |
...
สถานะ: R2A–R2D automated gate เสร็จแล้ว; Apps Script runtime/manual gate และ deployment ยังไม่เริ่ม
...
- [ ] ทำ Apps Script runtime/manual verification ก่อน deploy R2
...
| R2 Apps Script runtime gate | เจ้าของโครงการ | ยังไม่เริ่ม | — | ต้องทำ controlled verification ก่อนพิจารณา deploy R2 |
