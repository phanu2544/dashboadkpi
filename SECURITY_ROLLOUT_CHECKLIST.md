...
| R2 | Phase 1B — Session foundation | เพิ่ม server session และ auth helper กลาง | 2 | กำลังดำเนินการ — R2A primitives commit แล้ว; ยังไม่ push/deploy | R1 |
...
|---|---|
| Active release | R2 |
| Active phase | R2A — Session primitives |
| เป้าหมาย | สร้าง server-issued session foundation โดยยังไม่เปิด protected endpoint กลับมา |
| สถานะ implementation | R2A เสร็จแล้วใน `Code.js`; ยังไม่เชื่อมกับ `loginCheck` หรือ frontend |
| สถานะ review | Static checks และ in-memory session lifecycle tests ผ่าน |
| สถานะ commit | `5db29a656b612d7759ee37d18af6dd932e1dd93d` — `security: add server session primitives` |
| สถานะ Apps Script source | Production ยังเป็น R1; R2A ยังไม่ได้ `clasp push` |
| สถานะ verification | create/validate/role refresh/revoke/idle expiry/session limit ผ่าน |
| สถานะ Manual Test Gate | ยังไม่เริ่มสำหรับ R2 |
| สถานะ deployment | R1 อยู่บน production; R2 ยังไม่ได้ deploy |
| Blocker | R2A ไม่มี blocker; R2B ยังต้องเพิ่ม login throttling และกำหนด public session response contract |
| งานถัดไป | Push branch ขึ้น GitHub แล้ววางแผน R2B login/logout integration |
...
สถานะ: กำลังดำเนินการ — R2A เสร็จและ commit แล้ว; R2B–R2E ยังไม่เริ่ม
